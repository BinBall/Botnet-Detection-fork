...
flags = tf.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('partition_method', 'Metis', 'Method of Graph Partition(Metis/Louvain)')
...
def load_data(data_prefix, dataset_str, precalc, partition_method, train_ratio):
    ...
    return (train_adj, full_adj, train_feats, test_feats, y_train, y_val, y_test,
            ...
            num_data, visible_data, train_graph, full_graph)
...
def main(unused_argv):
    ...
    tf.logging.info(f'Start {FLAGS.partition_method} Partition...')
    if FLAGS.bsize > 1:
        if FLAGS.partition_method == "Louvain":
            _, parts = partition_utils.louvain_partition_graph(train_graph, train_adj, visible_data)
        else:
            _, parts = partition_utils.metis_partition_graph(train_adj, visible_data, FLAGS.num_clusters)
        ...
        (parts, features_batches, support_batches, y_train_batches,
         train_mask_batches) = utils.preprocess(train_adj, train_feats, y_train,
                                                ...
                                                FLAGS.num_clusters,
                                                FLAGS.diag_lambda,
                                                FLAGS.partition_method, train_graph)
    parts = [np.array(pt, dtype=np.int32) for pt in parts if len(pt) > 0]

    if FLAGS.partition_method == "Louvain":
        FLAGS.num_clusters = len(parts)
    tf.logging.info(f'{FLAGS.partition_method} Partition done, num_clusters: {FLAGS.num_clusters}')

    preprocess_time = time.time()
    tf.logging.info(f'Start eval_data preprocess...')
    (_, val_features_batches, val_support_batches, y_val_batches,
     val_mask_batches) = utils.preprocess(full_adj, test_feats, y_val,
                                          ...
                                          FLAGS.diag_lambda,
                                          FLAGS.partition_method, full_graph)
    tf.logging.info(f'Start test_data preprocess...')
    (_, test_features_batches, test_support_batches, y_test_batches,
     test_mask_batches) = utils.preprocess(full_adj, test_feats, y_test,
                                           ...
                                           FLAGS.diag_lambda,
                                           FLAGS.partition_method, full_graph)
    tf.logging.info(f'Preprocess done, {time.time() - preprocess_time:.2f} seconds')
    ...
    with tf.device('/cpu:0'):
        ...
        tf.logging.info(print_str)
        tf.logging.info(f'Partition_method:{FLAGS.partition_method}, total_epoch:{FLAGS.epochs}')
        tf.logging.info(f'Learning_rate:{FLAGS.learning_rate}, num_GCN:{FLAGS.num_layers}, hidden1:{FLAGS.hidden1}')
        tf.logging.info(
            f'batch_size:{FLAGS.bsize}, num_cluster_validate:{FLAGS.num_clusters_val}, num_cluster_test:{FLAGS.num_clusters_val}')
